...
TRAINER = __import__('02_train_dissentangle_model')
...
def objective(trial, optuna_path, logger, num_epochs=5):
    ...
    general_params = config.load_exp_config_file(exp_path=optuna_path)

    # creating exp_directory
    exp_path = os.path.join(optuna_path, f"trial_{trial_number}")
    utils.create_directory(exp_path)
    config.create_exp_config_file(exp_path=exp_path)
    exp_params = config.load_exp_config_file(exp_path=exp_path)
    ...
    # adjusting values
    exp_params = optuna_utils.suggest_values(trial, optuna_path, exp_params)
    config.save_exp_config_file(exp_path=exp_path, exp_params=exp_params)
    ...
    try:
        ...
        metric = trainer.training_losses["reconstruction"][-1]
    except:
        return 1e8
...
def optuna_optimization(exp_path, num_epochs=5, num_trials=50):
    ...
    if(not os.path.exists(values_file)):
        ...
        config.create_exp_config_file(exp_path)
        return
