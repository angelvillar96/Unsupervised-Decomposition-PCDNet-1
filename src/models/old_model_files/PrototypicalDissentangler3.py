...
class DecompModel3(nn.Module):
    ...
    def _greedy_selection(self, x, templates, masks):
        ...
        with torch.no_grad():
            for l in range(L):
                aux_objects_ = aux_objects.unsqueeze(1).repeat(1, T+1, 1, 1, 1, 1)
        ...
        objects = torch.stack([templates[i, idx] for i,idx in enumerate(template_ids)])
        masks = torch.stack([masks[i, idx] for i,idx in enumerate(template_ids)])
    ...
    def forward(self, x):
        ...
        for i, proto in enumerate(protos):
            ...
            if(self.randomness and torch.rand(1) > self.randomness_prob and self.training):
                proto = proto + (torch.rand(*proto.shape, device=device) - 0.5)  # TODO: scale noise
                # proto = proto
        ...
        mask_templates = tflib.apply_transform(imgs=masks, transform=translation_tf)
        ...
        if(self.color_transformer):
            templates = self.color_transformer(img=x, templates=templates, masks=mask_templates)

        # greedy selection
        self.templates = templates
        self.mask_templates = mask_templates
        objects, masks, object_ids = self._greedy_selection(x, templates, mask_templates)
